    ...
    return export_service.download_response(
        ...
        rows=[
            [
                ...
                item.category.name,
                item.brand.name if item.brand else None,
                item.unit,
                item.pack_size,
                item.base_mrp,
                item.on_hand,
...
async def bulk_products(
    ...
    current_user: Annotated[AdminUser, Depends(require_permission("products.view"))],
    session: Annotated[AsyncSession, Depends(get_db_session)],
) -> BulkActionResult:
    return await data_controls_service.bulk_products(session, payload, current_user)
