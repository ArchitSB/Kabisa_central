...
from app.services import coupon_service, data_controls_service, export_service, reporting_service
...
async def export_coupons(
    ...
    result = await coupon_service.list_coupons(
        session,
        page=1,
        page_size=None,
...
async def bulk_coupons(
    ...
    current_user: Annotated[AdminUser, Depends(require_permission("coupons.view"))],
    session: Annotated[AsyncSession, Depends(get_db_session)],
) -> BulkActionResult:
    return await data_controls_service.bulk_coupons(session, payload, current_user)
