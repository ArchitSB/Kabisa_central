...
async def delete_order(
    ...
    *,
    ...
    if order.status not in {OrderStatus.PENDING, OrderStatus.CANCELLED}:
        raise AppError(
            ...
            detail=(
                ...
                "Cancel committed orders instead."
            ),
    ...
    if payment_count:
        raise AppError(
            status_code=status.HTTP_409_CONFLICT,
            detail="Orders with recorded payments cannot be deleted.",
...
async def list_orders(
    ...
    *,
    ...
    statement = base.options(selectinload(Order.items)).order_by(ordering, Order.id.desc())
